...
def Game():
    ...
    def score(score):
        global ec, running, vikram_death_key
        text = tmd.render("Score:"+str(score),True,(255,255,255))
        
        screen.blit(text,[700,10])
        if (score%37 == 0) and score!=1 and score != 0:
            ec -= 1
            if ec < 1:
                ec = 1
        #print(ec)
    ...
    def create_enemy(e_c_c, e_c_t):
        global enemy_creation_count, enemy_creation_time, ec
        #print(e_c_c, e_c_t)
        if e_c_c > e_c_t and len(enemy_list) < 6:
             #print(1)
